...
class TestSireRcePeriodoFlow(SireRceTestMixin, TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def test_camino_a_check_accept_poll(self):
        ...
        self.assertEqual(periodo.last_ticket_id.rce_periodo_id, periodo)
        self.assertEqual(periodo.last_ticket_id.cod_libro, "080000")
        self.assertEqual(
            periodo.local_state,
            "checked",
    ...
    def test_download_summary_is_synchronous_no_ticket(self):
        """El servicio 5.35 "descargar resumen" es sincrono (buffer binario
        ...
        self.assertIn(
            "/1/0/exporta",
            url,
            "tipoReporte debe enviarse como digito positivo simple, por "
            "analogia con el mismo riesgo que RVIE ya sufrio (error SUNAT "
            "1056 por un valor con signo negativo) -- NO confirmado en "
            "vivo para RCE, ver plan aprobado.",
        )
