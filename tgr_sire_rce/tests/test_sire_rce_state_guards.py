"""Guards de estado LOCAL de ``sire.rce.periodo`` -- deben bloquear la
accion SIN llamar a SUNAT (``assert_not_called``), replicando los mensajes
1005/1008/1009 documentados en
``tgr_sire_rce/models/sunat_rce_tables.py::SIRE_RCE_ERROR_MESSAGES``.
"""
...
from odoo.addons.tgr_sire_rce.models.sunat_rce_tables import SIRE_RCE_ERROR_MESSAGES
...
class TestSireRceStateGuards(SireRceTestMixin, TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def test_register_preliminary_blocked_from_draft(self):
        ...
        self.assertEqual(str(ctx.exception), str(SIRE_RCE_ERROR_MESSAGES[1005]))
    ...
    def test_register_preliminary_blocked_when_already_registered(self):
        ...
        with patch(MOCK_PATH) as mocked, self.assertRaises(UserError) as ctx:
            periodo.action_register_preliminary()
        mocked.assert_not_called()
        self.assertEqual(str(ctx.exception), str(SIRE_RCE_ERROR_MESSAGES[1008]))
    ...
    def test_register_preliminary_blocked_when_closed(self):
        ...
        with patch(MOCK_PATH) as mocked, self.assertRaises(UserError) as ctx:
            periodo.action_register_preliminary()
        mocked.assert_not_called()
        self.assertEqual(str(ctx.exception), str(SIRE_RCE_ERROR_MESSAGES[1009]))
