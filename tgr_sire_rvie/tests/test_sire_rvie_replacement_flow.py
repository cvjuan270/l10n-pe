...
class TestSireRvieReplacementFlow(SireRvieTestMixin, TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def _create_wizard(self, periodo, filename="reemplazo.zip", content=FAKE_ZIP_BYTES):
        return self.env["sire.rvie.replacement.wizard"].create(
            {
    ...
    def test_upload_replacement_then_register_preliminary(self):
        ...
        create_response = sire_mock_response(
            201, headers={"Location": "/uploads/rvie-1"}
        )
        ...
        self.assertTrue(ticket.tus_location.endswith("/uploads/rvie-1"))
    ...
    def test_upload_replacement_network_error_is_caught_not_raised(self):
        """Ver decision de diseno 7 del plan aprobado: si la subida TUS
